...
public class ExpresssoinValidateUtil {
	...
	// 验证信用卡有效期 
	public static boolean isValidityDate(String inputString) {
		pattern = Pattern.compile("^[0-1][0-9][/][0-9][0-9]");
		macher = pattern.matcher(inputString);
		return macher.find();
	}
	...
	public static boolean isLegalPwd(String str){
		pattern = Pattern.compile("(?!^(\\d+|[a-zA-Z]+|[~!@#$%^&*?`()_=+|\\[\\]{}/\\;’:”.<>-]+)$)^[\\w~!@#\\$%\\^\\&\\*\\?`\\(\\)_=\\+\\|\\[\\]\\{}/\\\\;’:”.<>-]+$");
